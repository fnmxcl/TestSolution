...
module.exports.shell_200100103_DataQuery1_CheckDataValid = function ($context){
    ...
    var JsonParam = JSON.stringify({"TabularType":"ApplicationData.T_CM_SubscribeInfo","Properties":["CarType1","CarType2","CarType3","CHName","Province","City","Area","DataSource","Remark","ApplyTime","CarBrand","CarSerial","CarModel","WebSource","Status","BusinessMan","TraceResult","ClueTraceSerialNo","LinkTel","T_CM_ClueTraceInfoSeriaNo.NextTraceDate","T_CM_ClueTraceInfoSeriaNo.TraceContent","T_Province.CityName","T_Province.CityID","T_City.CityName","T_City.CityID","T_Area.CityName","T_Area.CityID","ExComeDT"],"Filter":"'杨柳'.Contains(BusinessMan)&&PointCode=='05719999' && (TraceResult!='失控'||TraceResult==null)null","PageSize":10,"StartIndex":0,"SortDescriptions":[{"PropertyName":"ApplyTime","Direction":"Descending"}]});
    try{
            ...
            if(res.status != 200){
                ...
                result.then(function(result){
                    ...
                    if(result.Status && result.Status == "Error" ){
                        ...
                        if(result.TotalItemCount != 2){
                            $context.incorrect("返回的数据集长度不为2!");
                            return;
                        }
                        if(result.Items[0].CHName != "马强"){
                            $context.incorrect("返回的数据集中第一个客户不为马强!");
                            return;
                        }
                        if(result.Items[1].CHName != "封魔"){
                            $context.incorrect("返回的数据集中第二个客户不为马强!");
                            return;
                        }
